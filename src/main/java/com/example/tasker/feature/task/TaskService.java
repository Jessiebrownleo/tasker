...
import com.example.tasker.feature.attachment.AttachmentRepository;
...
import com.example.tasker.feature.comment.CommentRepository;
...
public class TaskService {
    ...
    public TaskDetail createInColumn(String email, Long columnId,
        ...
        ColumnEntity column =
                columnEntityRepository.findById(columnId).orElseThrow();
        ...
        task.setPosition(request.getPosition()==null ?(max==null?1: max+1):
                request.getPosition());
    ...
    public TaskDetail move(String email,Long taskId,MoveTaskRequest request){
        ...
        ColumnEntity target =
                columnEntityRepository.findById(request.getToColumnId()).orElseThrow();
        ...
        task.setPosition(request.getPosition()==null ? (max==null? 1:max) :
                request.getPosition());
        return toDetail(task);
    }
    ...
    public List<TaskSummary> search(String email,String q){
        List<Task> results = taskRepository.search(q==null || q.isBlank()?
                null : q);
